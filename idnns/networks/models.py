...
def multi_layer_perceptron(x, n_input, n_classes, n_hidden_1, n_hidden_2):
    ...
    layer_1_input = tensorflow.add(tensorflow.matmul(x, weights['h1']), biases['b1'])
    layer_1 = tensorflow.nn.relu(layer_1_input)
    input.append(layer_1)
    hidden.append(layer_1)
    # Hidden layer with RELU activation
    layer_2_input = tensorflow.add(tensorflow.matmul(layer_1, weights['h2']), biases['b2'])
    layer_2 = tensorflow.nn.relu(layer_2_input)
    input.append(layer_2_input)
    hidden.append(layer_2)
    # Output layer with linear activation
    input_y = tensorflow.matmul(layer_2, weights['out']) + biases['out']
    y_output = tensorflow.nn.softmax(input_y)
...
def deepnn(x):
    ...
    with tensorflow.name_scope('conv1'):
        ...
        with tensorflow.name_scope('activation'):
            input_con1 = conv2d(x_image, W_conv1) + b_conv1
            h_conv1 = tensorflow.nn.relu(input_con1)
            tensorflow.summary.histogram('activations', h_conv1)
        with tensorflow.name_scope('max_pol'):
            # Pooling layer - downsampled by 2X.
    ...
    with tensorflow.name_scope('conv2'):
        ...
        with tensorflow.name_scope('activation'):
            ...
            h_conv2 = tensorflow.nn.relu(input_con2)
            tensorflow.summary.histogram('activations', h_conv2)
        with tensorflow.name_scope('max_pol'):
    ...
    with tensorflow.name_scope('FC1'):
        ...
        with tensorflow.name_scope('activation'):
            input_fc1 = tensorflow.matmul(h_pool2_flat, W_fc1) + b_fc1
            h_fc1 = tensorflow.nn.relu(input_fc1)
            tensorflow.summary.histogram('activations', h_fc1)

    with tensorflow.name_scope('dropout'):
        keep_prob = tensorflow.placeholder(tensorflow.float32)
        tensorflow.summary.scalar('dropout_keep_probability', keep_prob)
    ...
    input_y_conv = tensorflow.matmul(h_fc1_drop, W_fc2) + b_fc2
    y_conv = tensorflow.nn.softmax(input_y_conv)
