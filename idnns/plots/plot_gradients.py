...
def plot_gradients(name_s=None, data_array=None, figures_dir=''):
    ...
    for layer in range(0, num_of_layers):
        ...
        for epoch_index in range(num_of_epochs):
            # the gradients are dimensions of #batchs X # output weights - when output weights is the number of weights
            ...
            for batch_index in range(num_of_batchs):
                ...
                # Sum the covarince matrixes over the batches
...
def calc_mean_var_loss(epochsInds, loss_train):
    ...
    # Go over the epochs
    for epoch_index in range(num_of_epochs):
        # The loss for the specific epoch
        current_loss = loss_train[epoch_index, :]
        # The derivative between the batches
        ...
        # The covarince between the batches
...
def plot_loss_figures(data_array, fig_size=(14, 10), xlim=None, y_lim=None):
    ...
    p_5, = axes_log1.plot(epochsInds[1:], np.array(diff_mean_loss) / np.sqrt(dif_var_loss[1:]), linewidth=3,
                          color='green',
                          )
