...
def calc_information_sampling(data, bins, pys1, pxs, unique_inverse_x, unique_inverse_y):
    bins = bins.astype(np.float32)
...
def calc_information_for_layer_with_other(data, bins, unique_inverse_x, unique_inverse_y, pxs, pys1):
    local_IXT, local_ITY = calc_information_sampling(data, bins, pys1, pxs, unique_inverse_x, unique_inverse_y)
...
def calc_information_for_epoch(iter_index, interval_information_display, ws_iter_index, bins, unique_inverse_x,
                               unique_inverse_y, label, b, b1, len_unique_a, pys, pxs, py_x, pys1, model_path,
                               input_size, layerSize, calc_vartional_information=False,
                               calc_information_by_sampling=False, calc_full_and_vartional=False,
                               calc_regular_information=True, num_of_samples=100, sigma=0.5, ss=[], ks=[]):
...
def extract_probabilities(label, x):
...
def get_information(ws, x, label, num_of_bins, interval_information_display, model, layerSize, calc_parallel=True,
    ...
    pys, pys1, p_y_given_x, b1, b, unique_a, unique_inverse_x, unique_inverse_y, pxs = extract_probabilities(label, x)
    if calc_parallel:
        params = np.array(Parallel(n_jobs=NUM_CORES
                                     ...
                                     (i, interval_information_display, ws[i], bins, unique_inverse_x, unique_inverse_y,
                                      label, b, b1, len(unique_a), pys, pxs, p_y_given_x, pys1, model.save_file,
                                      x.shape[1], layerSize)
                                     for i in range(len(ws))))
    else:
        params = np.array([calc_information_for_epoch
                           (i, interval_information_display, ws[i], bins, unique_inverse_x, unique_inverse_y,
                            label, b, b1, len(unique_a), pys, pxs, p_y_given_x, pys1, model.save_file, x.shape[1],
                            layerSize)
                           for i in range(len(ws))])
    return params
